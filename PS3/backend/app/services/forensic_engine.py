...
class ForensicEngine:
    ...
    def detect_partitions(self, mount_point: str) -> List[Dict]:
        ...
        try:
            ...
            # Fallback: Image might be a raw volume (no partition table)
            logger.warning(f"mmls failed, attempting fallback fls volume check: {str(e)}")
            output = ""
        ...
        # If no partitions were found via mmls, try to detect if it's a raw volume (no partition table)
        if not partitions:
            try:
                # Check root directory at offset 0
                fls_cmd = [self.fls, str(ewf_file)]
                fls_result = self._run_command(fls_cmd)
                
                if '$MFT' in fls_result.stdout or '$Boot' in fls_result.stdout:
                    file_size = ewf_file.stat().st_size
                    partitions.append({
                        'slot': 0,
                        'start_offset': 0,
                        'end_offset': int(file_size / 512) - 1,
                        'length_sectors': int(file_size / 512),
                        'size_bytes': file_size,
                        'filesystem_type': 'NTFS',
                        'description': 'Raw Volume (No Partition Table)'
                    })
            except ForensicEngineError:
                raise ScanError("Partition detection failed: Image has no partition table and is not a recognizable raw NTFS volume.")
