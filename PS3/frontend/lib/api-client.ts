...
/**
 * Wipe Detection APIs (Phases 1-4)
 */
export async function extractArtifacts(evidenceId: number, partitionId: number) {
  const params = new URLSearchParams({
    evidence_id: evidenceId.toString(),
    partition_id: partitionId.toString()
  })
  return apiCall(`/artifacts/extract?${params.toString()}`, { method: 'POST' })
}

export async function getArtifacts(evidenceId: number) {
  return apiCall(`/artifacts/${evidenceId}`, { method: 'GET' })
}

export async function runWipeAnalysis(evidenceId: number, partitionId: number, minSuspicion: number = 50) {
  const params = new URLSearchParams({
    evidence_id: evidenceId.toString(),
    partition_id: partitionId.toString(),
    min_suspicion: minSuspicion.toString()
  })
  return apiCall(`/wipe/analyze?${params.toString()}`, { method: 'POST' })
}
